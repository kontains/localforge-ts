...
async function loadPrompt(templateName: string, data: any | null = null): Promise<string> {
...
let MAIN_SYSTEM_PROMPT: string = '';
let TOPIC_DETECTION_PROMPT: string = '';
let WHIMSICAL_GERUND_PROMPT: string = '';

// Initialize prompts - called when module is loaded
async function initializePrompts(config: { agentName?: string } = {}): Promise<void> {
    try {
        // Default agent name is "Jeff" but can be overridden
        const agentName = config.agentName ?? "Jeff";
...
initializePrompts({agentName: "Jeff"});

/**
 * Gets environment information for the context
 * @param workDir - Working directory path
 */
async function getEnvironmentInfo(workDir: string | null = null): Promise<string> {
 ...
 * @param workDir - Working directory path
 */
async function getDirectoryStructure(workDir: string | null = null): Promise<string> {
 ...
 * @param workDir - Working directory path
 */
async function getGitStatus(workDir: string | null = null): Promise<string> {
    // If no directory is set, return a message indicating this
    if (!workDir) {
 ...
 */
function estimateTokens(str: string | null = ''): number {
    return Math.ceil((str || '').length / 4.91);
}

/**
 * Calculates the current token count from a history array
 * @param historyArray - Array of message objects
 * @returns Estimated token count
 */
function calculateCurrentTokensFromHistory(historyArray: any[] = []): number {
 ...
 * @param workDir - Working directory path
 */
async function getSystemAndContext(workDir: string | null = null, sessionData: any = null): Promise<any[]> {
 ...
 */
async function detectTopic(message: string): Promise<{ isNewTopic: boolean, title?: string }> {
    try {
        const messages = [
            {
                role: 'system',
 ...
 */
async function generateGerund(word: string): Promise<string> {
    try {
        const messages = [
            {
                role: 'system',
 ...
 * @param currentMessages - Current message list for the LLM
 * @param agentTools - Object with getSchemas and run methods
 * @param workingDirectory - Working directory path
 * @param streamCallback - Optional callback for streaming updates
 * @param signal - Optional abort signal for interrupting execution
 * @returns Final LLM message
 */
async function runAgentLoop(
    ...
    streamCallback: ((update: any) => void) | null = null, 
    ...
    while (true) {
        ...
        if (hasToolCalls) {
            ...
            for (const call of llmResponse.tool_calls) {
                ...
                // but if it was an image we need to add temporary user message to delete it later, because tools can't carry multimodal data
...
function cleanupLoopHistoryFromImageMessage(loopMessages: any[]): void {
...
async function handleRequest(
    ...
    try {
        ...
        let imageDescriptionPromise: Promise<string> | null = null;
        if (userImageData) {
